...
from authentication.models import AddressDetail, AcademicInfo, UserInfo
from master_forms.models import City, District, SubCast, StreamOrField, Course, Semesters, Sections, Gender, UserType, \
    YearofAdmission, State, Tehsil, MotherTongue, Religion, BloodGroup, Nationality, Category, Reserved, \
    ApplyingConcession, Cast, PhysicallyChallenged, Degree, DegreeStreamOrField, TwelvethOrDiploma, UserOperations, \
    Subjects
...
@csrf_exempt
def get_citys_district_list(request):
...
def admin_signup(request):
    ...
    obj_semesters = Semesters.objects.all()
    obj_sections = Sections.objects.all()
    ...
    return render(request, 'homepage_signup.html',
                  {"obj_course": obj_course, "obj_semesters": obj_semesters, "obj_sections": obj_sections,
                   "obj_gender": obj_gender, "obj_user_type": obj_user_type, "obj_year_of_admission": obj_year_of_admission})
...
def register_user(request):
    ...
    if UserInfo.objects.filter(email=email).exists():
        ...
        if registered_as_text == "Student":
            obj_academicinfo = AcademicInfo(fk_user_info_id=obj_user_info.id, fk_course_id=course,
                                            fk_year_of_admission_id=admission_year, fk_sections_id=sections,
                                            fk_semesters_id=semesters)
            obj_academicinfo.save()
        else:
            pass
...
def profile_user(request):
    ...
    if session:
        ...
        if user_info_obj.fk_user_type.user_type == "Student":
            ...
            return render(request, "student_profile.html", {"user_info_obj": user_info_obj, \
                                                            "academic_info_obj": academic_info_obj, \
                                                            "state_obj": state_obj, \
                                                            "city_obj": city_obj, \
                                                            "gender_obj": gender_obj, \
                                                            "mother_tongue_obj": mother_tongue_obj, \
                                                            "religion_obj": religion_obj, \
                                                            "blood_group_obj": blood_group_obj, \
                                                            "nationality_obj": nationality_obj, \
                                                            "twelveth_or_diploma_obj": twelveth_or_diploma_obj, \
                                                            "stream_or_field_obj": stream_or_field_obj,
                                                            "district_obj": district_obj, "tehsil_obj": tehsil_obj,
                                                            "address_detail_obj": address_detail_obj,
                                                            "category_obj": category_obj, "reserved_obj": reserved_obj,
                                                            "applying_concession_obj": applying_concession_obj,
                                                            "cast_obj": cast_obj, "sub_cast_obj": sub_cast_obj,
                                                            "physically_challenged_obj": physically_challenged_obj,
                                                            "degree_obj": degree_obj,
                                                            "degree_stream_or_field_obj": degree_stream_or_field_obj})
        else:
            user_operations_obj = UserOperations.objects.filter(fk_user_role_id=user_info_obj.fk_user_role.id)
            subjects_obj = Subjects.objects.all()
            # specialization_obj = Specialization.objects.all()
            print ("faculty")
            return render(request, "faculty_profile.html", {"user_info_obj": user_info_obj, \
                                                            "subject_obj": subjects_obj, \
                                                            # "specialization_obj":specialization_obj,\
                                                            "state_obj": state_obj, \
                                                            "city_obj": city_obj,
                                                            "gender_obj": gender_obj,
                                                            "mother_tongue_obj": mother_tongue_obj,
...
def forgot_password(request):
    ...
    if UserInfo.objects.filter(email=input_forget_password).exists():
        ...
        email.send()
        send_data = {"status": "1",
                     "msg": "Your new password has been sent on your registred email id, please check your email and re-login"}
    else:
...
def save_personal_info(request):
    ...
    address_detail_obj.correspondence_pin_code = request.POST.get("correspondence_pincode")
...
def save_academic_details(request):
    ...
    academic_info_obj.tenth_year_of_passing = datetime.datetime.strptime(str(request.POST.get("date_of_school_passing")),
    ...
    academic_info_obj.college_date_of_passing = datetime.datetime.strptime(str(request.POST.get("college_date_of_passing")),
                                                                          '%d-%m-%Y').strftime('%Y-%m-%d')
...
def upload_profile(img):
    ...
    imgN = "profile_images/" + "Profile_" + current + '.jpg'
